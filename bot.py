...
def parse_phone_list(value: str) -> Set[str]:
    ...
    To improve matching
    accuracy, only digits are retained when checking numbers.  Country codes
    are not stripped here; matching happens by suffix to handle local
    differences.
    """
    result: Set[str] = set()
    if not value:
...
class ShellBot:
    ...
    def handle_start(self, update: Update, context: CallbackContext) -> None:
        ...
        button = KeyboardButton(
            "Share your phone number", request_contact=True
        )
    ...
    def handle_contact(self, update: Update, context: CallbackContext) -> None:
        ...
        if contact is None or contact.user_id != user_id:
            message.reply_text(
                "Error: please use the button to share your number."
            )
            return
        ...
        if matched:
            ...
            logger.info(
                "User %s authorized via phone number %s", user_id, phone_number
            )
        else:
            message.reply_text(
                "Phone number not authorized. Access denied."
            )
            logger.warning(
    ...
    def handle_command(self, update: Update, context: CallbackContext) -> None:
        ...
        if user_id not in self.allowed_user_ids and user_id not in self.authorised_users:
            update.message.reply_text(
                "You are not authorized. Use /start to authenticate."
            )
            return
        ...
        try:
            # Execute the command in a subprocess; use shell=True to allow pipelines and complex commands
...
def main() -> None:
    ...
    if not token:
        raise RuntimeError(
            "TELEGRAM_BOT_TOKEN (or TELEGRAM_BOT_TOKEN_FILE) is not set."
        )
